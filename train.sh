...
use_qat=True
qat_w_bits=4
use_quantized_rollout=True

# Validate QAT configuration
if [ "$use_qat" != "True" ] && [ "$use_qat" != "False" ]; then
    echo "Error: use_qat must be either True or False" && exit 1
fi
if [ "$use_quantized_rollout" != "True" ] && [ "$use_quantized_rollout" != "False" ]; then
    echo "Error: use_quantized_rollout must be either True or False" && exit 1
fi

if [ "$use_quantized_rollout" = "True" ] && [ "$use_qat" = "False" ]; then
    echo "Error: Cannot use quantized rollout without QAT" && exit 1
fi
