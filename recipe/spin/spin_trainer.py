...
from verl.utils.torch_functional import masked_mean  # noqa: E402
...
def _compute_response_info(batch: DataProto) -> Dict[str, Any]:
    ...
    try:
        ...
        if "attention_mask" in batch.batch and "response_mask" in batch.batch:
            full_mask = batch.batch["attention_mask"]  # noqa: F841
            resp_mask = batch.batch["response_mask"]  # noqa: F841
...
def compute_dpo_data_metrics(batch: DataProto) -> Dict[str, Any]:
    ...
    try:
        ...
        if "preferences" in batch.batch and batch.batch["preferences"] is not None:
            prefs_mask = batch.batch["preferences"]  # Shape [batch_size * n]  # noqa: F841
...
def apply_kl_penalty(data: DataProto, kl_ctrl: core_algos.AdaptiveKLController, kl_penalty="kl"):
    ...
    kld = core_algos.kl_penalty(data.batch["old_log_probs"], data.batch["ref_log_prob"], kl_penalty=kl_penalty)  # (batch_size, response_length)  # type: ignore
...
class RaySPINTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if not self.async_rollout_mode:
                ...
                self.async_rollout_manager.wake_up()  # type: ignore
                test_output_gen_batch_padded = self.async_rollout_manager.generate_sequences(test_gen_batch_padded)  # type: ignore
                self.async_rollout_manager.sleep()  # type: ignore
        ...
        if val_data_dir:
            self._dump_generations(  # type: ignore
    ...
    def init_workers(self):
        ...
        for resource_pool, class_dict in self.resource_pool_to_cls.items():
            ...
            wg_dict = self.ray_worker_group_cls(resource_pool=resource_pool, ray_cls_with_init=worker_dict_cls, **wg_kwargs)  # type: ignore
    ...
    def fit_dpo(self):  # Renamed for clarity as standard PPO loop
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            if hasattr(self.train_dataloader, "reset"):
                try:
                    self.train_dataloader.reset()  # type: ignore
                except Exception as e:
