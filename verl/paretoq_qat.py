...
class LsqBinaryTernaryExtension(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx: Any, input: Tensor, alpha: Tensor, num_bits: int, layerwise: bool) -> Tensor:
        """
    ...
    def backward(ctx: Any, grad_output: Tensor) -> tuple[Tensor, Tensor, None, None]:
        ...
        if ctx.num_bits == 1:
            ...
            if layerwise:
                ...
                grad_alpha = (indicate_small * Qn + indicate_big * Qp + indicate_middle * (-q_w + q_w.round())) * grad_output * grad_scale
                grad_alpha = torch.sum(grad_alpha, dim=-1, keepdim=True)
...
class StretchedElasticQuant(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.num_bits == 1:
            ...
            if layerwise:
                grad_alpha = ((indicate_small * Qn + indicate_big * Qp + indicate_middle * (-q_w + (torch.round(torch.clamp(q_w, -clip_val, clip_val) * n_levels - shift) + shift) / n_levels)) * grad_output * grad_scale).sum().unsqueeze(dim=0)
            else:
                grad_alpha = (indicate_small * Qn + indicate_big * Qp + indicate_middle * (-q_w + (torch.round(torch.clamp(q_w, -clip_val, clip_val) * n_levels - shift) + shift) / n_levels)) * grad_output * grad_scale
                grad_alpha = torch.sum(grad_alpha, dim=-1, keepdim=True)
...
# --------------- NEW ---------------
def patch_linear_with_quantized_linear(linear: nn.Linear, w_bits: int = 16, weight_layerwise: bool = False) -> None:
    linear.w_bits = w_bits
    linear.weight_layerwise = weight_layerwise
    if w_bits < 16:
        linear.weight_clip_val = nn.Parameter(torch.Tensor(linear.weight.shape[0], 1).to(linear.weight.device, linear.weight.dtype))
        with torch.no_grad():
            if w_bits == 1:
            ...
            linear.weight_clip_val.data.copy_(scale)

    linear.forward = QuantizeLinear.forward.__get__(linear, type(linear))


def replace_linear_with_quantized_linear(module: nn.Module, w_bits: int = 16, weight_layerwise: bool = False) -> None:
...
def get_fake_quantized_state_dict(model: nn.Module) -> dict[str, Tensor]:
    fake_quantized_state_dict = {}
    for key, tensor in model.state_dict().items():
        if key.endswith(".weight"):
            module = model.get_submodule(key[: -len(".weight")])
            if hasattr(module, "w_bits") and hasattr(module, "weight_layerwise"):
                with torch.no_grad():
                    assert len(tensor.size()) == 2
                    if module.w_bits >= 16:
                        fake_quantized_weight = tensor
                    elif module.w_bits == 2 or module.w_bits == 0:
                        fake_quantized_weight = StretchedElasticQuant.apply(
                            tensor,
                            module.weight_clip_val,
                            module.w_bits,
                            module.weight_layerwise,
                        ).to(tensor.dtype)
                    elif module.w_bits <= 4:
                        fake_quantized_weight = LsqBinaryTernaryExtension.apply(
                            tensor,
                            module.weight_clip_val,
                            module.w_bits,
                            module.weight_layerwise,
                ...
                fake_quantized_state_dict[key] = fake_quantized_weight
            else:
                fake_quantized_state_dict[key] = tensor
        else:
            fake_quantized_state_dict[key] = tensor

    return fake_quantized_state_dict
