...
class HQQFSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __init__(self, module: FSDP, inference_engine_full_precision: LLM, inference_engine_hqq: LLM, model_config, full_params: bool = False, device_mesh: DeviceMesh = None, offload_param: bool = False, load_format: str = "dummy_hf", layered_summon: bool = True): # type: ignore
        self.module = module
        # For AsyncLLM, inference_engine and model_runner are defer initialized in vLLMAsyncRollout.load_model
        self.inference_engine_full_precision = inference_engine_full_precision
        self.inference_engine_hqq = inference_engine_hqq
        ...
        self.model_runner_full_precision = self.inference_engine_full_precision.llm_engine.model_executor.driver_worker.worker.model_runner if self.inference_engine_full_precision else None
        self.model_runner_hqq = self.inference_engine_hqq.llm_engine.model_executor.driver_worker.worker.model_runner if self.inference_engine_hqq else None
    ...
    def __enter__(self):
        ...
        with _timer("reshard", self.timing):
            ...
            if vllm_version in (
                ...
                if "tags" in inspect.signature(self.inference_engine_full_precision.wake_up).parameters:
                    self.inference_engine_full_precision.wake_up(tags=["weights"])
                else:
                    self.inference_engine_full_precision.wake_up()

                if "tags" in inspect.signature(self.inference_engine_hqq.wake_up).parameters:
                    self.inference_engine_hqq.wake_up(tags=["weights"])
                else:
                    self.inference_engine_hqq.wake_up()
                ...
                if "tags" in inspect.signature(self.inference_engine_full_precision.wake_up).parameters:
                    self.inference_engine_full_precision.wake_up(tags=["kv_cache"])
                else:
                    self.inference_engine_full_precision.wake_up()

                if "tags" in inspect.signature(self.inference_engine_hqq.wake_up).parameters:
                    self.inference_engine_hqq.wake_up(tags=["kv_cache"])
                else:
                    self.inference_engine_hqq.wake_up()
    ...
    def update_params(self, updated_params, peft_config=None):
        ...
        loaded_params = full_precision_model.load_weights(
            (
                (name, param.to(device, non_blocking=True).full_tensor() if isinstance(param, DTensor) else param) # type: ignore
                for name, param in updated_params.items()
            )
        )

        full_precision_state_dict = full_precision_model.state_dict()

        with torch.no_grad():
            for name, param in hqq_model.named_parameters():
                if name[-len(".W_q"):] == ".W_q":
                    quant_layer = hqq_model.get_submodule(name[:-len(".W_q")])
                    quant_params = {
                        k: v 
                        for k, v in quant_layer.quant_config["weight_quant_params"].items()
                        if k not in ["channel_wise", "optimize", "round_zero"]
                    }
                    param.set_(
                        quant_layer.from_weights(
                            weight=full_precision_state_dict[name[:-len(".quant_layer.W_q")] + ".weight"],
                            bias=None,
                            quant_config=BaseQuantizeConfig(**quant_params),
                        ).W_q
                    )
                else:
                    param.set_(full_precision_state_dict[name])
