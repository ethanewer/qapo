...
def _repeat_interleave(value: Union[torch.Tensor, np.ndarray], repeats: int) -> Union[torch.Tensor, List[Any], np.ndarray]:
...
class HQQvLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        delta_position_id = delta_position_id.unsqueeze(0).expand(batch_size, -1)  # type: ignore
        if position_ids.dim() == 3:  # type: ignore # qwen2vl mrope
            delta_position_id = delta_position_id.view(batch_size, 1, -1).expand(batch_size, 3, -1)  # type: ignore
...
class HQQvLLMAsyncRollout:
    ...
    def wake_up(self, *args, **kwargs):
        ...
        if not self.is_sleep:
            return
        self.sharding_manager.__enter__()  # type: ignore # pylint: disable=C2801
        self.is_sleep = False
